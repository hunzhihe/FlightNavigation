...
TArray<FAStarNode> UOctreeFlightComponent::InitializeGenerateFlightNavMesh()
{
	...
	TArray<FAStarNode> VoxelGrid = UFlightNavigationBFL::GenerateVoxelGrid(GetWorld(), NavMeshMinBounds, NavMeshMaxBounds, NodeSize);
	
	for (FAStarNode& Node : VoxelGrid)
	{
		VoxelGrids.Add(Node.Location, Node);
	}
	...
	for ( auto ObstructionBox : BanFlightNavMeshBoundsVolumes)
	{
        ...
        for (int32 X = MinX; X < MaxX; ++X)
        {
             for (int32 Y = MinY; Y < MaxY; ++Y)
             {
                 ...
                 {
                     // 构造 Voxel 的索引键（FIntVector）
                     //FVector VoxelKey(X, Y, Z);
                ...
                // {
                    ...
                    //else
                    //{
                        // 如果该 Voxel 还不存在，但又在障碍范围内，你也可以选择创建它并设为不可通行
                        // FAStarNode NewVoxel;
                        // NewVoxel.Location = VoxelCenter;
                        // NewVoxel.bIsWalkable = false;
                        // VoxelGrids.Add(VoxelKey, NewVoxel);
                        // 可选：DrawDebugVoxelBlocked(VoxelCenter);
                    //}
                //}
            }
        }
